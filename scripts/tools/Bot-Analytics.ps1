...
# ── Coverage mode: delegates to Run-CoverageGap.ps1 for full analysis ──
if ($BotType -eq "coverage") {
    $coverageScript = Join-Path $PSScriptRoot 'Run-CoverageGap.ps1'
    & $coverageScript -RepoRoot $repoRoot -OutputDir $OutputDir
    Pop-Location
    exit $LASTEXITCODE
}
