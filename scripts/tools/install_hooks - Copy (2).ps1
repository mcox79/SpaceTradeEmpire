...
# Mark the sh hook as executable in the index for environments that care.
try {
	& git update-index --chmod=+x ".githooks/pre-commit" 2>$null | Out-Null
} catch { }

# Configure repo-local hooks path (per-clone .git/config).
& git config core.hooksPath ".githooks" | Out-Null

Write-Host "Installed repo-tracked hooks in .githooks"
Write-Host "Set core.hooksPath=.githooks (repo-local per clone)"
Write-Host "Pre-commit will run: scripts/check_tabs.ps1"
