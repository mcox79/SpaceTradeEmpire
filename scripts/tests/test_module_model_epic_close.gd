				...
				for sv in slots2:
					if typeof(sv) == TYPE_DICTIONARY:
						if str(sv.get("slot_id", "")) == "slot_weapon_0" and str(sv.get("installed_module_id", "")) == "weapon_laser_mk1":
							...
							print(PREFIX + "MOD_CLOSE|PASS")
							_quit()
							return false

			_polls += 1
			if _polls >= MAX_POLLS:
				print(PREFIX + "FAIL: equip not applied after timeout")
				_quit()
