...
public partial class StationMenu : Control
{
        ...
        private void SetupUI()
        {
                ...
                vbox.AddChild(new Label { Text = "MARKET (Buy/Sell + Programs)", Modulate = new Color(0.7f, 0.7f, 1f) });
                ...
                _bridge.ExecuteSafeRead(state =>
                {
                                ...
                                foreach (var good in sortedGoods)
                                {
                                        ...
                                        if (string.IsNullOrWhiteSpace(progId))
                                        {
                                                ...
                                                btnCreate.Pressed += () =>
                                                {
                                                        ...
                                                        _bridge.CreateAutoBuyProgram(_currentMarketId, good, quantity: 1, cadenceTicks: 10);
                                                };
                        ...
                        else
                        {
                                foreach (var v in sus)
                                {
                                        ...
                                        string starveBand = d.ContainsKey("starve_band") ? d["starve_band"].ToString() : "?";
                                        string failBand = d.ContainsKey("fail_band") ? d["fail_band"].ToString() : "?";
...
...
                                                        string covBand = id.ContainsKey("coverage_band") ? id["coverage_band"].ToString() : "?";

                                                        _sustainmentList.AddChild(new Label
                                                        {
                                                                Text = $"  - {good}: have {have}, req/t {perTick}, target {target}, cover {covBand}, margin {m:0.00}"
                                                        });
