...
# Pipeline buttons
$y = 105
$btnGenAll = New-DevtoolButton $tabPipeline "0. GENERATE ALL (1 + 2 + 3)" 10 $y 375 40 "#444444" {
...
$btnTest = New-DevtoolButton $tabPipeline "1. VERIFY LOGIC (Tests)" 10 $y 375 40 "#007acc" {
    Run-SimCoreTests
    Set-StatusText
}
$y += 45

$btnContext = New-DevtoolButton $tabPipeline "2. PHASE 1: CONTEXT PACKET" 10 $y 375 40 "#6a00ff" {
...
$btnLedgerIr = New-DevtoolButton $tabPipeline "2b. PHASE 1b: LEDGER IR PROMPT (LLM)" 10 $y 375 40 "#555555" {
    Run-LedgerIrPrompt -Cap 8 | Out-Null
    Set-StatusText
}
...
$btnScan = New-DevtoolButton $tabPipeline "3. PHASE 1: CONNECTIVITY SCAN" 10 $y 375 40 "#d46a00" {
    Run-ConnectivityScan | Out-Null
    Set-StatusText
}
$y += 45

$btnBuildQueue = New-DevtoolButton $tabPipeline "4. PHASE 2: BUILD QUEUE (Stage 2)" 10 $y 375 40 "#444444" {
    $cap = Get-QueueCapFromUi -TextBox $txtQueueCap
...
$btnApplyQueue = New-DevtoolButton $tabPipeline "5. PHASE 2b: APPLY QUEUE TO REGISTRY" 10 $y 375 40 "#444444" {
    $ok = Run-ApplyQueue
...
$btnValidate = New-DevtoolButton $tabPipeline "5b. VALIDATE REGISTRY (Validate-Gates)" 10 $y 375 40 "#333333" {
    Run-ValidateRegistry | Out-Null
    Set-StatusText
}
...
$btnGitStatus = New-DevtoolButton $tabExec "9. GIT STATUS" 10 $y2 375 40 "#333333" {
    Run-GitStatus | Out-Null
    Set-StatusText
}
$y2 += 45

$btnCommitPush = New-DevtoolButton $tabExec "10. GIT COMMIT + PUSH" 10 $y2 375 40 "#228822" {
...
$lblExecNote.Text = "Note: Shard Execution uses docs/gates/gates.json as registry."
$lblExecNote.Location = New-Object System.Drawing.Point(10, $y2)
$lblExecNote.Size = New-Object System.Drawing.Size(375, 40)
