...
- EPIC.S7.SHIP_PROGRESSION.V0 [TODO]: Ship purchase system — 10 faction ship variants per faction_equipment_and_research_v0.md Part 4, ShipyardSystem (purchase/sell-back at stations), progressive catalog disclosure (faction rep + systems visited), T2 module faction reassignment, balance pass, NPC faction fleet assignment, fleet roster management, ship comparison panel (gates: GATE.T59.SHIP.*)
...
- EPIC.S6.DISCOVERY_TRADE_PIPELINE.V0 [TODO]: Exploration->automation pipeline wiring — EconomicIntel entity (typed variants per discovery family), DISCOVERY_OPPORTUNITY FO trigger, intel decay->margin buffer, NPC route competition (margin compression 300-500t), SurveyV0 program. P0-P1 prerequisites for FO Trade Manager Phase 2. Fixes ACTIVE_ISSUES E4. Per ExplorationDiscovery.md §Implementation Roadmap (gates: GATE.T57.PIPELINE.*)
...
- EPIC.T52.AUDIT_FIXES.V0 [IN_PROGRESS]: Audit-driven fixes from full game audit (2026-03-23) — economy trade diversity (E4), tariff wiring, upkeep tuning, dread exposure scaling + secondary stressors, narrative dialogue extraction to JSON + voice differentiation + sequence variants, NPC combat labels + target highlight + hitstop, discovery UI (scanner vis, phase markers, breadcrumbs, milestone cards). 23 gates across 5 epics. (gates: GATE.T52.*)
