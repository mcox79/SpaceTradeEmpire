...
- EPIC.S7.GALAXY_MAP_V2.V0 [TODO]: Galaxy map evolution — 5 new overlay modes (Faction Territory, Exploration, Fleet Positions, Warfronts, Heat), route planner with multi-hop path display, galaxy search (find system by name), semantic zoom (detail levels by altitude), constellation clustering, icon toggles. Per GalaxyMap.md. GameShell-only; SimBridge queries: GetFactionTerritoryOverlayV0, GetFleetPositionsV0, GetWarfrontOverlayV0, GetHeatOverlayV0, GetRouteV0, GetSystemSearchV0 (gates: GATE.S7.GALAXY_MAP_V2.*)
- EPIC.S7.NARRATIVE_DELIVERY.V0 [TODO]: Narrative text delivery system — add flavor_text/description fields to DiscoveryStateV0, JumpEvent, AnomalyEncounter; build text display panel with faction voice styling; discovery narrative templates from ExplorationDiscovery.md wired into DiscoveryOutcomeSystem; faction station greeting text keyed to reputation tier; discovery milestone feedback cards (scan complete / analysis complete). Per NarrativeDesign.md + content specs in docs/design/content/NarrativeContent_TBA.md. Hash-affecting for entity field additions (gates: GATE.S7.NARRATIVE_DELIVERY.*)
...
- EPIC.S7.FLEET_TAB.V0 [TODO]: Empire Dashboard Fleet tab (F3) — master-detail fleet list with per-fleet cargo, installed modules, assigned programs, status, doctrine; action buttons (assign program, change doctrine, view modules). Pulled forward from S9 — fleet management is core gameplay, not polish. Per EmpireDashboard.md. GameShell-only (gates: GATE.S7.FLEET_TAB.*)
- EPIC.S7.MAIN_MENU.V0 [TODO]: Main menu game shell — main_menu.tscn as project main scene (replacing direct playable_prototype boot), adaptive menu list (Continue first when saves exist, New Voyage first otherwise), new voyage wizard (captain name, galaxy seed, 3-tier difficulty with SimCore multipliers for sustain/tariff/combat/trace), save slot metadata enhancement (screenshot thumbnail capture, playtime accumulation, captain name, difficulty, current system), auto-save slot (slot 0, triggers on dock/warp/mission-step), pause menu upgrades (Save and Quit single action, Settings button, quit-with-time-since-last-save warning), scene transition flow (menu -> Main.tscn with seed+difficulty via autoload globals). Per MainMenu.md. SimCore changes: difficulty multiplier fields in SimState, captain name persistence. GameShell + SimBridge + SimCore (gates: GATE.S7.MAIN_MENU.*)
...
- EPIC.S7.RISK_METER_UI.V0 [TODO]: Risk meter visualization per RiskMeters.md — Heat/Influence/Trace widgets in Zone G, 5 named thresholds, trend arrows, decay rate display, threshold transition toasts, screen-edge ambient tinting at High+, compound threat indicators. GameShell-only; SimBridge queries: GetRiskMetersV0, GetRiskDecayRateV0, GetCompoundThreatV0 (gates: GATE.S7.RISK_METER_UI.*)
...
- EPIC.S7.GALAXY_MAP_V2.V0 [TODO]: Galaxy map evolution per GalaxyMap.md — 5 new overlay modes (Faction Territory/Exploration/Fleet Positions/Warfronts/Heat), route planner, galaxy search, semantic zoom, constellation clustering, icon toggles. GameShell-only; SimBridge queries: GetFactionTerritoryOverlayV0, GetFleetPositionsV0, GetWarfrontOverlayV0, GetHeatOverlayV0, GetRouteV0, GetSystemSearchV0 (gates: GATE.S7.GALAXY_MAP_V2.*)
- EPIC.S7.NARRATIVE_DELIVERY.V0 [TODO]: Narrative text delivery system per NarrativeDesign.md — add flavor_text/description fields to DiscoveryStateV0/JumpEvent/AnomalyEncounter; text display panel with faction voice styling; discovery narrative templates; faction station greetings; discovery milestone feedback cards. Content specs in docs/design/content/NarrativeContent_TBA.md. Hash-affecting for entity fields (gates: GATE.S7.NARRATIVE_DELIVERY.*)
...
- EPIC.S7.FLEET_TAB.V0 [TODO]: Empire Dashboard Fleet tab (F3) per EmpireDashboard.md — master-detail fleet list, per-fleet cargo/modules/programs/status/doctrine, action buttons. Pulled forward from S9 — fleet management is core gameplay. GameShell-only (gates: GATE.S7.FLEET_TAB.*)
- EPIC.S7.MAIN_MENU.V0 [TODO]: Main menu game shell per MainMenu.md — main_menu.tscn as project main scene, adaptive menu list, new voyage wizard (name/seed/difficulty), save slot enhancement (thumbnails/playtime/captain), auto-save slot, pause menu upgrades (Save and Quit, quit-with-time-warning), difficulty multipliers in SimCore. GameShell + SimBridge + SimCore (gates: GATE.S7.MAIN_MENU.*)

Status: IN_PROGRESS — Faction model, territory, embargo, instability, warfronts, reputation, planets, production chains, sustain enforcement, power budget, instability effects, starter placement, faction visuals, enforcement escalation, HUD architecture, combat juice, audio wiring all DONE (135 gates). Remaining: Warfront State (morale/attrition), Faction Identity Redesign, Tech Access, Diplomacy Verbs, UI (Diplo + Warfront), T2 Modules, Risk Meter UI, Galaxy Map V2, Narrative Delivery, Automation Management, Fleet Tab, Main Menu.
...
- EPIC.S9.SETTINGS.V0 [TODO]: Full options screen — 4 tabs (Gameplay, Display, Audio, Accessibility), auto-save on change, shared between main menu and pause menu. Per MainMenu.md. Depends on EPIC.S7.MAIN_MENU.V0 + EPIC.S7.AUDIO_WIRING.V0. GameShell-only (gates: GATE.S9.SETTINGS.*)
- EPIC.S9.MENU_ATMOSPHERE.V0 [TODO]: Title screen atmosphere — parallax starfield shader, adaptive foreground silhouette, menu audio timing, galaxy generation screen, Precursor subtitle quotes. Per MainMenu.md. GameShell-only (gates: GATE.S9.MENU_ATMOSPHERE.*)
- EPIC.S9.ACCESSIBILITY.V0 [TODO]: First-launch accessibility prompt, colorblind shaders, font size override, high contrast, reduced shake. Per MainMenu.md. GameShell-only (gates: GATE.S9.ACCESSIBILITY.*)
