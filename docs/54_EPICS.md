...
- EPIC.S8.HAVEN_STARBASE.V0 [IN_PROGRESS]: Hidden Precursor starbase per haven_starbase_v0.md — 5-tier upgrade tree, Haven Residents, Trophy Wall, Haven market, Hangar system, 20 data logs, Keeper system, Resonance Chamber, accommodation threads, endgame paths. **T33**: HavenStarbase entity, SeedHavenV0, HavenUpgradeSystem, HavenHangarSystem, deferred Haven market, SimBridge.Haven.cs, Haven dock panel, galaxy map icon. **T34**: Haven Residents (Keeper + FO candidates), Trophy Wall (fragment deposit + resonance pair display), 19 Haven data logs, Fragment Geometry display, Ancient Hull restoration at T3+, headless proof. **Remaining** (~35-60 gates per haven_starbase_v0.md): Keeper ambient system (5-tier evolution), accommodation threads (one-way/bidirectional), Resonance Chamber (fragment combination), Fabricator (T3 module manufacturing), Haven market evolution (tier-based stock expansion), Coming Home transitions (cinematic docking), endgame path mechanics (Reinforce/Naturalize/Renegotiate), Reveal Thread choice (Tier 4), Communion Representative (Tier 3+), visual geometry per tier. (gates: GATE.S8.HAVEN.*)
...
- EPIC.S8.NARRATIVE_CONTENT.V0 [IN_PROGRESS]: Authored story content. **T38**: FactionDialogueContentV0 (15 entries), WarfrontCommentaryContentV0 (25 entries), RevelationTextV0 (5 gold toasts). Remaining: Haven starbase logs, endgame path narratives, expanded fragment lore. Content specs in docs/design/content/NarrativeContent_TBA.md + docs/design/content/LoreContent_TBA.md (gates: GATE.S8.NARRATIVE.*)
...
Status: IN_PROGRESS — 45 DONE epics in S7. Remaining TODO/IN_PROGRESS: Faction Model, Faction Identity Redesign, Combat Depth V2, Faction Commission, Territory Shift, Tech Access, Diplomacy Verbs, UI (Diplo + Warfront), Bridge Throneroom, Class Warfront Profiles.
...
- EPIC.S8.MEGAPROJECTS [IN_PROGRESS]: Multi-stage projects that reshape map rules under supply constraints. **T41**: Megaproject entity, MegaprojectSystem (start/deliver/process/mutate), MegaprojectContentV0 (3 types: fracture_anchor, trade_corridor, sensor_pylon), map rule mutations (IsFractureNode, SpeedMultiplierPct, SensorPylonNodes), DeliverMegaprojectSupplyCommand, StartMegaprojectCommand, SimBridge.Megaproject.cs (5 queries), megaproject_panel.gd (M key), 9 contract tests + 4 integration tests. Remaining: megaproject visual markers on galaxy map, construction VFX, more types (gates: GATE.S8.MEGAPROJECTS.*)
...
- EPIC.S8.HAVEN_STARBASE.V0 [IN_PROGRESS]: Hidden Precursor starbase in stable Phase-0 space per haven_starbase_v0.md; dormant until player docks; one-way outbound secret lane initially; 5-tier upgrade tree (Powered/Inhabited/Operational/Expanded/Awakened); bidirectional travel at tier 3; Haven Residents; Trophy Wall; Haven market; Hangar; 20 data logs; lore delivery hub; strategic asset for all three endgame paths. **T33**: HavenStarbase entity, SeedHavenV0, HavenUpgradeSystem, HavenHangarSystem, deferred Haven market, SimBridge.Haven.cs, dock panel, galaxy icon. **T34**: Haven Residents (Keeper + FO candidates), Trophy Wall (deposit + resonance display), Fragment Geometry display, 19 Haven data logs, Ancient Hull restoration, headless proof. **T37**: Keeper 5-tier ambient evolution (Dormant→Awakened), Resonance Chamber (fragment pair combination + cooldown), Fabricator (T3 module fabrication + exotic matter cost), Haven market periodic restocking, SimBridge.Haven depth queries (keeper/resonance/fabricator/market). **T41**: HavenResearchLabSystem (tier-gated parallel research slots 1/2/3 at T2/T3/T4), TransferModuleCommand (drydock module transfer at T3+), accommodation thread bonuses (Discovery/Commerce/Conflict/Harmony at 33/66/100%), RevealHavenCommand (T4+ one-time faction ally reveal + rep boost). **Remaining**: Coming Home transitions, visual geometry per tier, Communion Representative dialogue. **Prerequisite for**: EPIC.S8.WIN_SCENARIOS + EPIC.S8.T3_PRECURSOR_MODULES.V0 (tier 3+ fabrication). Per factions_and_lore_v0.md + haven_starbase_v0.md (gates: GATE.S8.HAVEN.*)
...
- EPIC.S8.NARRATIVE_CONTENT.V0 [IN_PROGRESS]: Authored story content for the story state machine. **T38**: FactionDialogueContentV0 (15 entries, 5 factions x 3 rep tiers), WarfrontCommentaryContentV0 (25 entries, 5 factions x 5 intensities), AdaptationFragmentContentV0 cover/revealed lore pairs, RevelationTextV0 (5 gold toast entries). Remaining: Haven starbase logs, endgame path narratives, expanded fragment lore. Content specs in docs/design/content/NarrativeContent_TBA.md + docs/design/content/LoreContent_TBA.md (gates: GATE.S8.NARRATIVE.*)
- EPIC.T18.KNOWLEDGE_GRAPH_SEEDING.V0 [TODO]: Knowledge graph connection generation pipeline per knowledge_graph_mechanics_v0.md. 11 templates in KnowledgeGraphContentV0.AllTemplates exist but NO worldgen code resolves them — state.Intel.KnowledgeConnections is always empty. Template resolution ($KEPLER_1-6, $LOG.THREAD.NUM → discovery IDs), procedural proximity connections (BFS ≤2 hops), faction link generation, density validation (15-25 connections target). Call site in GalaxyGenerator after Phase 9 (data logs). The "?" bridge mechanic (Outer Wilds pattern) never fires without this. 2-3 gates. Hash-affecting (gates: GATE.T18.KG_SEED.*)
...
Epics:
- EPIC.S9.SAVE [IN_PROGRESS]: Robust save UX, migrations, corruption handling (lock final migration policy). **T41**: SaveEnvelope version-aware serialization (CurrentVersion=2), v1→v2 migration (Haven/Megaprojects/SensorPylonNodes), TryDeserializeSafe corruption detection, ValidateState post-load validation, GetSaveIntegrityV0 bridge. 11 migration+integrity tests. Remaining: auto-save, recovery UX, cloud save (gates: GATE.S9.SAVE.*)
