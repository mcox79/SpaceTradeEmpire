...
- EPIC.S9.MISSION_LADDER.V0 [IN_PROGRESS]: Missions M2-M6 introducing mining, patrol, construction, research, fracture. T29: Mining Survey, Ore Extraction, First Research, Research Materials, First Build, Station Expansion content + bridge extensions. Remaining: patrol mission, fracture mission content (gates: GATE.S9.MISSIONS.*)
- EPIC.S9.MISSION_FOUNDATION.V0 [TODO]: Mission system Phase 1 per mission_design_v0.md — 4 new trigger types (ReputationMin, CreditsMin, TechUnlocked, TimerExpired), non-credit rewards (reputation, access, intel), mission failure/abandonment with tick deadlines, CHOICE branching steps with conditional paths. Foundation for all future mission layers (systemic, template, faction storylines). Hash-affecting (gates: GATE.S9.MISSION_EVOL.*)
- EPIC.S9.SYSTEMIC_MISSIONS.V0 [TODO]: Mission system Phase 2 per mission_design_v0.md — procedural mission generator reading world state (market shortages, warfront demand, discovery leads), station context system (dock shows situation not board), FO mission commentary hooks. Zero-authoring-cost missions that emerge from simulation (gates: GATE.S9.SYSTEMIC.*)
