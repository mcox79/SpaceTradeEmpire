...
- EPIC.S7.AUTOMATION_MGMT.V0 [TODO]: Automation management layer — program performance tracking (profitability, activity stats), failure reason UI with actionable explanations, program budget caps, doctrine system (fleet-wide behavior rules), program templates. Per AutomationPrograms.md. SimBridge queries: GetProgramPerformanceV0, GetProgramFailureReasonsV0, GetDoctrineSettingsV0 (gates: GATE.S7.AUTOMATION_MGMT.*)
...
- EPIC.S8.ADAPTATION_FRAGMENTS.V0 [TODO]: 16 Adaptation fragments + 8 resonance pairs, prerequisite for win scenarios. Content in docs/design/content/LoreContent_TBA.md (gates: GATE.S8.ADAPTATION.*)
- EPIC.S8.HAVEN_STARBASE.V0 [TODO]: Hidden Precursor starbase per haven_starbase_v0.md — 5-tier upgrade tree, Haven Residents, Trophy Wall, Haven market, Hangar system, 20 data logs (gates: GATE.S8.HAVEN.*)
...
- EPIC.S9.SYSTEMIC_MISSIONS.V0 [IN_PROGRESS]: Mission system Phase 2 per mission_design_v0.md — procedural mission generator reading world state (market shortages, warfront demand, discovery leads), station context system (dock shows situation not board), FO mission commentary hooks. Zero-authoring-cost missions that emerge from simulation (gates: GATE.S9.SYSTEMIC.*)
...
- EPIC.X.LEDGER_EVENTS.V0 [IN_PROGRESS]: Event-sourced economic ledger — CashDelta/InventoryDelta on every change (gates: GATE.X.LEDGER_EVENTS.*)
...
- EPIC.S7.COMBAT_PHASE2.V0 [IN_PROGRESS]: Phase 2 combat per ship_modules_v0.md and CombatFeel.md — Battle Stations spin-up mode, heat system (heat-per-shot, capacity, rejection rate, overheat cascade), radiator targeting/destruction, spinal mount fire. Heat gauge HUD widget. Extends StrategicResolverV0 and CombatSystem. Hash-affecting SimCore + GameShell (gates: GATE.S7.COMBAT_PHASE2.*)
...
- EPIC.X.LEDGER_EVENTS.V0 [IN_PROGRESS]: Event-sourced economic ledger — CashDelta and InventoryDelta events on every credit/goods change with ActorId, Category, ReasonCode; ledger integrity invariant (sum of CashDeltas = wallet delta); enables automation postmortems and "where did the money go?" reconstruction (gates: GATE.X.LEDGER_EVENTS.*)
...
Status: IN_PROGRESS — Combat local, playable, doctrine, NPC trade, escort, security lanes, combat loot all DONE (59 gates). Remaining: Loss Recovery, UI Security, Coupling Limits. PERF_BUDGET merged into EPIC.X.PERF_BUDGET.
...
- EPIC.S7.AUTOMATION_MGMT.V0 [TODO]: Automation management layer per AutomationPrograms.md — program performance tracking, failure reason UI, budget caps, doctrine system, program templates. SimBridge queries: GetProgramPerformanceV0, GetProgramFailureReasonsV0, GetDoctrineSettingsV0 (gates: GATE.S7.AUTOMATION_MGMT.*)
...
- EPIC.S8.HAVEN_STARBASE.V0 [TODO]: Hidden Precursor starbase in stable Phase-0 space per haven_starbase_v0.md; dormant until player docks; one-way outbound secret lane initially; 5-tier upgrade tree (Powered/Inhabited/Operational/Expanded/Awakened); bidirectional travel at tier 3; Haven Residents (The Keeper ancient construct, unpromoted FO candidates as secondary crew, Communion Representative at tier 3+); Trophy Wall + Fragment Geometry visual personalization; Haven market (exotic-only, never pentagon faction goods); Hangar system (max 3 ships); 20 data logs across 5 tiers; lore delivery hub; strategic asset for all three endgame paths. **Prerequisite for**: EPIC.S8.WIN_SCENARIOS + EPIC.S8.T3_PRECURSOR_MODULES.V0 (tier 3+ fabrication). Per factions_and_lore_v0.md + haven_starbase_v0.md (gates: GATE.S8.HAVEN.*)
