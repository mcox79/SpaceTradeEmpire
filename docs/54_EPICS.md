...
- EPIC.S5.TRACTOR_SYSTEM.V0 [IN_PROGRESS]: Tractor beam module system per ship_modules_v0.md — 3-tier module-gated range (T1 Magnetic Grapple 15u, T2 EM Tractor Array 30u auto-target, T3 Graviton Tether 50u grab disabled hulks), CollectLootCommand range check against equipped tractor module, visual tractor beam VFX (energy beam pulling loot), Weaver faction variant (Spindle Tractor 25u auto-salvage). **T33 DONE**: HavenTweaksV0 tractor ranges, 3 tractor module content entries, SimBridge GetTractorRangeV0, tractor beam VFX. **Remaining**: Weaver faction variant, auto-target logic. Extends EPIC.S5.COMBAT_LOOT.V0. SimCore + GameShell (gates: GATE.S5.TRACTOR.*)
...
- EPIC.S8.ADAPTATION_FRAGMENTS.V0 [TODO]: 16 Adaptation fragments + 8 resonance pairs, prerequisite for win scenarios. **T34 queued**: entity, collection system, worldgen placement, bridge+UI. Content in docs/design/content/LoreContent_TBA.md (gates: GATE.S8.ADAPTATION.*)
- EPIC.S8.HAVEN_STARBASE.V0 [IN_PROGRESS]: Hidden Precursor starbase per haven_starbase_v0.md — 5-tier upgrade tree, Haven Residents, Trophy Wall, Haven market, Hangar system, 20 data logs. **T33 DONE**: HavenStarbase entity (6 persisted fields), SeedHavenV0 (farthest node placement), HavenUpgradeSystem (resource-gated tier advancement), HavenHangarSystem (store/swap, tier-gated bays), deferred Haven market (tier-based exotic stock), SimBridge.Haven.cs (4 query methods), Haven dock panel, galaxy map icon. **Remaining**: Haven Residents, Trophy Wall, 20 data logs, Fragment Geometry. (gates: GATE.S8.HAVEN.*)
- EPIC.S8.LATTICE_DRONES.V0 [TODO]: Instability-phase-linked hostile Lattice drone NPCs (gates: GATE.S8.LATTICE_DRONES.*)
- EPIC.S8.NARRATIVE_CONTENT.V0 [TODO]: Authored story content — Adaptation Fragment lore text (16 entries), Haven starbase logs and upgrade narratives, NPC faction representative dialogue, warfront event commentary, fracture discovery revelation text, endgame path narratives. Delivery SYSTEM is in EPIC.S7.NARRATIVE_DELIVERY; this epic is CONTENT authoring only. Content specs in docs/design/content/NarrativeContent_TBA.md + docs/design/content/LoreContent_TBA.md (gates: GATE.S8.NARRATIVE.*)
- EPIC.S8.T3_PRECURSOR_MODULES.V0 [TODO]: ~13 T3 discovery-only modules, exotic matter sustain. **T34 queued**: content definitions, exotic sustain wiring, discovery-only acquisition gate, bridge+UI. (gates: GATE.S8.T3_MODULES.*)
...
- EPIC.S9.TEMPLATE_MISSIONS.V0 [TODO]: Mission system Phase 3 per mission_design_v0.md — 50-65 authored mission templates with procedural variables (supply/logistics, exploration, combat/security, reputation/politics), twist slot system, reward scaling formula (gates: GATE.S9.TEMPLATES.*)
...
- EPIC.S9.MUSIC.V0 [TODO]: Background soundtrack composition and dynamic music stems — ambient exploration music, combat music triggers, warfront tension, fracture ambience, discovery stingers. Audio bus architecture is in EPIC.S7.AUDIO_WIRING.V0; this epic is music COMPOSITION and stem authoring. Content specs in docs/design/content/AudioContent_TBA.md (gates: GATE.S9.MUSIC.*)
...
- EPIC.S9.MILESTONES_CREDITS.V0 [TODO]: Milestones viewer accessible from main menu — aggregate progression across all save slots, milestone card grid (achieved=full color+date, unachieved=silhouette), lifetime stats (total playtime, credits earned, goods traded, systems visited, voyages completed), per-voyage stat drill-down. Credits scroll over menu background with skip-on-input. Per MainMenu.md. Reads existing MilestoneSystem + PlayerStats via SimBridge. GameShell-only (gates: GATE.S9.MILESTONES_CREDITS.*)
...
- EPIC.X.COVER_STORY_NAMING.V0 [TODO]: Pre-revelation naming discipline per NarrativeDesign.md — "fracture" forbidden in player-facing text before Revelation 1 ("It's Not a Drive"). Module names use cover stories ("Structural Resonance Engine" not "Fracture Drive"), HUD labels and bridge snapshots switch post-revelation. CI grep enforcement on string literals. Depends on EPIC.S8.STORY_STATE_MACHINE for revelation trigger. Non-hash-affecting GameShell + SimBridge (gates: GATE.X.COVER_STORY.*)
...
- EPIC.S8.PENTAGON_BREAK.V0 [TODO]: Pentagon dependency cascade — the game's #1 narrative moment per NarrativeDesign.md. Player fracture-space trade route enables Communion Food self-production, breaking one dependency link. Downstream faction economic consequences cascade. Revelation delivery: gold toast "Trade Analysis Complete — Pattern Detected" + galaxy map pentagon highlight + FO reaction + Discovery Web gold ring. Depends on EPIC.T18.CHARACTER_SYSTEMS.V0 + EPIC.S8.STORY_STATE_MACHINE. Hash-affecting (gates: GATE.S8.PENTAGON_BREAK.*)
- EPIC.S8.ANCIENT_SHIP_HULLS.V0 [TODO]: Three ancient ship hulls per ship_modules_v0.md — Bastion (tank), Seeker (explorer), Threshold (fracture specialist). Discovered at deep void sites, restored at Haven Tier 3+. Pre-revelation names ("Hull Type XV-3"). Unique slot layouts and passive abilities. **T34 queued**: 3 hull defs, Haven T3+ restoration system, bridge+UI. Depends on EPIC.S8.HAVEN_STARBASE.V0. Hash-affecting (gates: GATE.S8.ANCIENT_HULLS.*)
...
- EPIC.X.PERF_BUDGET [IN_PROGRESS]: Tick budget tests across all system slices (security, exploration, warfront, endgame). Single profiling pass covering all SimCore systems, not per-slice. Replaces individual S5/S6/S7/S8 PERF_BUDGET epics (gates: GATE.X.PERF_BUDGET.*)
- EPIC.X.STATION_IDENTITY.V0 [TODO]: Station visual variety — per-faction color tinting, tier-based size scaling (outpost/hub/capital), visual distinction so stations are not all identical. Fixes ACTIVE_ISSUES F6. Non-hash-affecting GameShell (gates: GATE.X.STATION_IDENTITY.*)
...
- EPIC.X.COVER_STORY_NAMING.V0 [TODO]: Pre-revelation naming discipline per NarrativeDesign.md — "fracture" forbidden in player-facing text before Revelation 1. Cover names for modules/systems, CI grep enforcement. Depends on EPIC.S8.STORY_STATE_MACHINE. Non-hash-affecting (gates: GATE.X.COVER_STORY.*)
...
- EPIC.S5.TRACTOR_SYSTEM.V0 [TODO]: Tractor beam module system per ship_modules_v0.md — 3-tier module-gated loot collection (T1 Magnetic Grapple 15u basic materials, T2 EM Tractor Array 30u all loot + auto-target, T3 Graviton Tether 50u + grab disabled hulks for station salvage), CollectLootCommand checks equipped tractor module for range + loot type eligibility, visual tractor beam VFX (energy beam pulling loot toward ship), Weaver faction variant Spindle Tractor (25u auto-salvage per faction_equipment_and_research_v0.md). Without tractor module: manual proximity pickup only (5u). Extends EPIC.S5.COMBAT_LOOT.V0. SimCore (module gating) + GameShell (beam VFX) (gates: GATE.S5.TRACTOR.*)

Status: IN_PROGRESS — Combat local, playable, doctrine, NPC trade, escort, security lanes, combat loot all DONE (59 gates). Remaining: Loss Recovery, UI Security, Coupling Limits, Tractor System. PERF_BUDGET merged into EPIC.X.PERF_BUDGET.
...
- EPIC.S6.UI_DISCOVERY [TODO]: Discovery UI overhaul — discovery phase markers on galaxy map (gray/amber/green icons), scanner range visualization, discovery milestone feedback cards (scan complete/analysis complete per ExplorationDiscovery.md), Knowledge Graph / Discovery Web UI (Outer Wilds Ship Log-inspired relationship map), breadcrumb trail visualization, scanner sweep animation on system entry, active leads display. Per ExplorationDiscovery.md (gates: GATE.S6.UI_DISCOVERY.*)
...
Status: IN_PROGRESS — Fracture travel, commerce, econ invariants, discovery outcomes, galaxy map, Fracture Discovery Event all DONE (66 gates). Remaining: Anomaly Ecology, Artifact Research (needs T2/T3 modules), Tech Leads (needs T2/T3 modules), Science Center, Expedition fullness, UI Discovery, Layered Reveals completion. PERF_BUDGET merged into EPIC.X.PERF_BUDGET.
...
Status: IN_PROGRESS — 45 DONE epics in S7. Remaining TODO/IN_PROGRESS: Automation Management, Faction Identity Redesign, Combat Phase 2, Warfront State, Territory Regimes, Reputation Influence, Tech Access, Diplomacy Verbs, UI (Diplo + Warfront), Bridge Throneroom, Class Warfront Profiles.
...
- EPIC.S8.WIN_SCENARIOS [TODO]: Three endgame paths (Reinforce/Naturalize/Renegotiate) per factions_and_lore_v0.md, each with distinct gameplay requirements, faction reputation thresholds, and fragment prerequisites; explicit loss states (death/bankruptcy with narrative frames); epilogue system (45-90 sec text cards showing costs of paths not chosen); equipment state reflection in epilogue. Supersedes older containment/alliance/dominance terminology (gates: GATE.S8.WIN_SCENARIOS.*)
- EPIC.S8.UI_WARROOM [TODO]: Warfronts + policing + megaproject pipelines + bottlenecks (gates: GATE.S8.UI_WARROOM.*)
- EPIC.S8.STORY_STATE_MACHINE [TODO]: Story beats via discovery/trace/warfront phases, not timed missions; Five Recontextualizations delivery system — revelation triggers (module predates threads, threads aren't protection, economy is a cage, module is changing me, instability is not entropy), gold toast + galaxy map highlights + FO reactions + Discovery Web updates per NarrativeDesign.md; cover-story naming state transitions (pre/post-revelation name switching). Hash-affecting (gates: GATE.S8.STORY_STATE_MACHINE.*)
...
- EPIC.S8.HAVEN_STARBASE.V0 [IN_PROGRESS]: Hidden Precursor starbase in stable Phase-0 space per haven_starbase_v0.md; dormant until player docks; one-way outbound secret lane initially; 5-tier upgrade tree (Powered/Inhabited/Operational/Expanded/Awakened); bidirectional travel at tier 3; Haven Residents (The Keeper ancient construct, unpromoted FO candidates as secondary crew, Communion Representative at tier 3+); Trophy Wall + Fragment Geometry visual personalization; Haven market (exotic-only, never pentagon faction goods); Hangar system (max 3 ships); 20 data logs across 5 tiers; lore delivery hub; strategic asset for all three endgame paths. **T33 DONE**: HavenStarbase entity, SeedHavenV0, HavenUpgradeSystem, HavenHangarSystem, deferred Haven market, SimBridge.Haven.cs, dock panel, galaxy icon, 28 contract tests. **Remaining**: Haven Residents, Trophy Wall, Fragment Geometry, 20 data logs. **Prerequisite for**: EPIC.S8.WIN_SCENARIOS + EPIC.S8.T3_PRECURSOR_MODULES.V0 (tier 3+ fabrication). Per factions_and_lore_v0.md + haven_starbase_v0.md (gates: GATE.S8.HAVEN.*)
...
- EPIC.S8.NARRATIVE_CONTENT.V0 [TODO]: Authored story content for the story state machine — Adaptation Fragment lore text (16 entries), Haven starbase logs and upgrade narratives, NPC faction representative dialogue, warfront event commentary, fracture discovery revelation text, endgame path narratives. Delivery SYSTEM is in EPIC.S7.NARRATIVE_DELIVERY; this epic is CONTENT authoring only. Content specs in docs/design/content/NarrativeContent_TBA.md + docs/design/content/LoreContent_TBA.md (gates: GATE.S8.NARRATIVE.*)
...
- EPIC.S8.PENTAGON_BREAK.V0 [TODO]: Pentagon dependency cascade — the game's #1 narrative moment per NarrativeDesign.md. Player fracture-space trade route enables Communion Food self-production, breaking one dependency link. Downstream faction economic consequences cascade. Revelation delivery: gold toast "Trade Analysis Complete — Pattern Detected" + galaxy map pentagon highlight + FO reaction + Discovery Web gold ring. **Depends on**: EPIC.T18.CHARACTER_SYSTEMS.V0 + EPIC.S8.STORY_STATE_MACHINE. Hash-affecting (gates: GATE.S8.PENTAGON_BREAK.*)
- EPIC.S8.ANCIENT_SHIP_HULLS.V0 [TODO]: Three ancient ship hulls per ship_modules_v0.md — Bastion (tank), Seeker (explorer), Threshold (fracture specialist). Discovered at deep void sites, restored at Haven Tier 3+. Pre-revelation names ("Hull Type XV-3"). Unique slot layouts and passive abilities. **Depends on**: EPIC.S8.HAVEN_STARBASE.V0. Hash-affecting (gates: GATE.S8.ANCIENT_HULLS.*)
...
- EPIC.S9.MILESTONES_CREDITS.V0 [TODO]: Milestones viewer (aggregate stats across saves) + credits scroll. Per MainMenu.md. GameShell-only (gates: GATE.S9.MILESTONES_CREDITS.*)
- EPIC.S9.MUSIC.V0 [TODO]: Background soundtrack composition and dynamic music stems — ambient exploration music, combat music triggers, warfront tension, fracture ambience, discovery stingers. Audio bus architecture is in EPIC.S7.AUDIO_WIRING.V0; this epic is music COMPOSITION and stem authoring. Content specs in docs/design/content/AudioContent_TBA.md (gates: GATE.S9.MUSIC.*)
