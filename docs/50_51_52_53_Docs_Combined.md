...
> ## Implementation Phase Key (added 2026-03-08)
> Throughout this document, sections are annotated with their implementation phase:
> - **[PHASE 1 — IMPLEMENTED]**: Built and tested in SimCore. Code references provided.
> - **[PHASE 2 — NEXT]**: Planned for near-term tranches. Foundations exist.
> - **[PHASE 3 — ASPIRATIONAL]**: Long-term design target. No implementation yet.

## 50. Early Mission Ladder and Travel

> **Phase Status**: [PHASE 1] Mission M1 "Matched Luggage" + 3 tutorial missions implemented (MissionContentV0.cs, MissionSystem.cs). [PHASE 2] Missions M2-M6 (Mining, Patrol, Construction, Research, Fracture) not yet implemented — the mission-gated progression system that forces each core system is incomplete.
...
Canonical design guidance for early scripted missions, travel rules, and the off-thread exploration layer. Intended to drive implementation, tooling, and content authoring.
...
\- Thread booking UI (with capacity display)
...
\- Thread capacity is shown and explained, but does not block the tutorial mission.
...
\### M6: Fracture Drive (off-thread layer intro % mystery layer unlock)

Unlocks:

\- Practical off-thread exploration

\- New map layer of mysteries between threads
...
\- Practical off-thread traversal rules and constraints
...
\- Must not trivialize threads, capacity, and patrol gameplay.
...
2\) Book Star Thread to Destination
...
\### Thread fee policy for Mission 1 (no vouchers)

\- No voucher items unless vouchers become a permanent system later.

\- Thread fee exists and is shown.
...
\### Thread capacity policy for Mission 1

\- Capacity must be displayed and explained.

\- Tutorial thread must always have capacity to book successfully.

\- At least one other thread in the UI should be visibly full or congested to communicate scarcity without blocking progress.

\- UI text must explicitly state the tutorial thread is guaranteed for training.
...
\## 6. Threads vs off-thread travel rules

\### Thread travel

\- Threads are the empire backbone for logistics, capacity constraints, and route warfare.

\- Thread travel speed is massively higher than off-thread travel (example: 1000x off-thread cruise).



\### Off-thread travel (baseline, any ship)

\- Off-thread travel is allowed for any ship.

\- Off-thread baseline travel is intentionally impractical for interstellar movement (example: on the order of a week of game-time).

\- Design intent: continuous off-thread cruising is dead time and not a normal strategy.



\### Fracture drive travel (practical off-thread)

\- Fracture drive makes off-thread traversal practically useful.

\- Fracture drive has its own speed and constraints.

\- Fracture drive unlocks the “between threads” mystery layer.
...
\## 7. Off-thread guardrails (non-negotiable)

Because off-thread baseline travel can effectively self-lock a player, the game must include these protections:



\### 7.1 Off-thread commitment warning

\- If a player commits to off-thread travel that exceeds a configured threshold (in game-time), the game must show a clear warning and require confirmation.

\- Warning must include a clear statement that off-thread without fracture is intentionally impractical.



\### 7.2 Return mechanism (bounded, not free)

\- The player must have an always-available “Return to last thread entry” option.

\- It is not instant. It is an autopilot burn that consumes deterministic game-time.

\- It returns to a specific thread entry node, not “nearest safe place.”
...
\### 7.3 Persistent value for off-thread discoveries

Off-thread exploration must never be “you found something, but it is meaningless.”
...
Goal: fracture expands the game without collapsing thread relevance.
...
\### 10.5 Thread relevance protection

\- Threads remain the default logistics backbone.

\- Early game freighters remain thread-only.
...
\- book\_thread(route\_id, sponsor\_coverage\_id?)
...
\- Thread booking required for tutorial cannot fail due to capacity.
...
\- accept % book\_thread (sponsor pays) % travel % dock % deliver % buy % return % sell % unlock freighters
...
> **Phase Status**: This section describes the full economic simulation target. Current implementation covers Phase 1 (basic trade with supply/demand curves, 13 goods, 9 recipes, NPC trade). Phase 2 (fuel burn, storage limits, bid/ask, module sustain) and Phase 3 (event-sourced ledger, mass/volume, decay, substitution, NPC parity) are not yet implemented.
...
\- Logistics: ports, threads, convoys, storage
...
\- Shocks change state (capacity offline, piracy capacity up, tariff schedule change, thread closure, labor strike).
...
> **[PHASE 3 — ASPIRATIONAL]**: Event-sourced ledger (CashDelta, InventoryDelta, TradeFill) is not implemented. Commands directly mutate state.PlayerCredits and state.PlayerCargo without emitting structured events. The "all money movement must be explainable" requirement is aspirational.
...
> **[PHASE 1]**: 13 goods defined (WellKnownGoodIds.cs), 9 recipes (ContentRegistryLoader.cs). **[PHASE 3]**: Physical model (unit_mass, unit_volume, handling_class, decay_model, substitution_group, economic_role) not implemented — cargo is unitless integer counts.
...
> **[PHASE 1]**: Thread capacity (LaneFlowSystem). **[PHASE 2]**: Fuel burn for fleet travel, storage limits per station. **[PHASE 3]**: Berth queues, service time, loading/unloading throughput, labor pools. Currently logistics is NOT the primary constraint — only thread capacity bottlenecks exist.
...
> **[PHASE 1]**: Supply/demand curves (Market.cs), price publishing every 720 ticks, transaction fees (100 bps). **[PHASE 2]**: Bid/ask spread, depth-dependent pricing. **[PHASE 3]**: Volatility-driven spread widening, NPC arbitrage, exponential smoothing, hysteresis.
...
> **[PHASE 1]**: NPC trade circulation (NpcTradeSystem.cs — autonomous traders evaluate markets, execute profitable trades). **[PHASE 3]**: NPC capital pools, risk tolerances, reaction time, information freshness limits, volume limits. NPCs do not currently run the same economic loops as the player.
...
\- If the player discovers a high-margin thread, NPC volume follows unless the player has a defensible moat (permits, relationships, infrastructure, intelligence).
...
> **[PHASE 1]**: 8 program kinds (ProgramKind.cs: AutoBuy, AutoSell, TradeCharter, ResourceTap, Expedition, ConstrCap, Patrol, Escort). Basic program execution via ProgramSystem + IntentSystem. **[PHASE 2]**: Failure taxonomy, execution penalties, confidence/staleness inputs. **[PHASE 3]**: Full policy execution with risk bands, liquidity reserves, exposure caps, objective weights.
...
> **[PHASE 1]**: Industry maintenance (MaintenanceSystem.cs — site decay, repair costs, supply consumption). Transaction fees. Tariffs. **[PHASE 2]**: Crew payroll, insurance premiums. **[PHASE 3]**: Facility OPEX, financing costs, depreciation, replacement cycles.
...
> **[PHASE 1]**: Risk/security threads (RiskSystem.cs, SecurityLaneSystem.cs — probabilistic inspections, delays, cargo loss). Edge.Heat tracking on cargo volume. **[PHASE 2]**: Pattern-based enforcement, confiscation mechanics, fine system. **[PHASE 3]**: Market manipulation detection, contraband categories, jurisdiction models.
...
\- Thread closures and security state deterioration
...
\### 16.3 Abstract thread flow determinism (required)

If the world uses “abstract thread flow” (aggregate transport rather than ship-per-ship), it must be deterministic:
...
> **Phase Status**: [PHASE 1 — IMPLEMENTED]. All development locks in this section are respected. Tick = 1 game minute, 60x time scale, tick boundary rule, no acceleration modes, 720-tick publish cadence — all enforced in SimKernel and GameShell.
...
1\) Abstract thread flow:

\- per thread capacity and delay
...
\- Abstract thread flow must be deterministic (pure function of state + seed) and must not depend on entity spawn order or frame timing.
...
\- 2 stations, 1 thread, 2 goods

\- station inventories, production/consumption

\- abstract thread flow with delay
...
> **Phase Status**: [PHASE 1] 8 program kinds, fleet roles (Trader/Hauler/Patrol), basic doctrine fields, ProgramQuote. [PHASE 2] Fleet Groups, Upgrade Packages, doctrine-driven dispatch behavior, failure taxonomy. Player Ship fitting works but modules are not market-tradeable goods.
