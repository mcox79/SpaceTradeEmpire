...
> **[PHASE 1]**: Lane capacity (LaneFlowSystem). **[PHASE 2]**: Fuel burn for fleet travel, storage limits per station. **[PHASE 3]**: Berth queues, service time, loading/unloading throughput, labor pools. Currently logistics is NOT the primary constraint — only lane capacity bottlenecks exist.
...
> **[PHASE 1]**: Risk/security lanes (RiskSystem.cs, SecurityLaneSystem.cs — probabilistic inspections, delays, cargo loss). Edge.Heat tracking on cargo volume. **[PHASE 2]**: Pattern-based enforcement, confiscation mechanics, fine system. **[PHASE 3]**: Market manipulation detection, contraband categories, jurisdiction models.
