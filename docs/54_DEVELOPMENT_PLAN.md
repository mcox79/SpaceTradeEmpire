...
| GATE.LOGI.EXEC.001 | LogisticsJob executes end-to-end across ticks under kernel order: travel to source, issue load once (latched), travel to target, issue unload once (latched), clear job; no double-issue while idle at node | DONE | SimCore/Systems/LogisticsSystem.cs + SimCore/Entities/LogisticsJob.cs + SimCore.Tests/Systems/LogisticsJobExecutionIntegrationTests.cs |
